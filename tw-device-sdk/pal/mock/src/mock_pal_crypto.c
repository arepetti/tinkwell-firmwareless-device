/*
 * mock_pal_crypto.c -- SHA-256 mock using the POSIX implementation.
...
/*
 * Reuse the portable SHA-256 from the POSIX PAL.
 * For unit tests this is good enough.
 */
...
tw_err_t pal_sha256_finish(pal_sha256_ctx_t *ctx, uint8_t digest[PAL_SHA256_DIGEST_SIZE])
{
    uint8_t pad[128];
...
tw_err_t pal_crypto_sha256(const void *data, size_t len, uint8_t digest[PAL_SHA256_DIGEST_SIZE])
{
    pal_sha256_ctx_t ctx;
    pal_sha256_init(&ctx);
    pal_sha256_update(&ctx, data, len);
    return pal_sha256_finish(&ctx, digest);
}
