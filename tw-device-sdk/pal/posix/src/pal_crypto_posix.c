/*
 * pal_crypto_posix.c -- SHA-256 via a minimal public-domain implementation.
 *
 * Uses a self-contained SHA-256 so the POSIX build needs no external
 * crypto library.  For production host builds, link against OpenSSL or
 * libsodium and replace this file.
 *
                           ...
                           uint8_t digest[PAL_SHA256_DIGEST_SIZE])
{
    ...
    for (int j = 0; j < 8; j++)
        pad[pad_len - 1 - j] = (uint8_t)(bits >> (j * 8));

    for (size_t p = 0; p < pad_len; p += 64)
        sha256_transform(ctx->state, pad + p);
